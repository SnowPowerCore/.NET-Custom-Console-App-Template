...
public class VersionTrackingService : IVersionTrackingService
{
    ...
    public VersionTrackingService(ILocalStorageService localStorage)
    {
        ...
        CurrentVersion = typeof(VersionTrackingService).Assembly.GetName().Version?.ToString();
        CurrentBuild = typeof(VersionTrackingService).Assembly.GetName().Version?.Build.ToString();
